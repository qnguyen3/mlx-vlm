...
class Idefics3PerceiverAttention(nn.Module):
...
class Idefics3PerceiverLayer(nn.Module):
    def __init__(self, config: ModelConfig):
        super().__init__()
        ...
        self.self_attn = Idefics3PerceiverAttention(config)
...
class Idefics3PerceiverResampler(nn.Module):
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.hidden_size = config.text_config.hidden_size
        self.n_latents = config.perceiver_config.resampler_n_latents
...
class Idefics3Connector(nn.Module):
    def __init__(self, config: ModelConfig):
        super().__init__()
        self.modality_projection = MLP(config.text_config.hidden_size, bias=False)
